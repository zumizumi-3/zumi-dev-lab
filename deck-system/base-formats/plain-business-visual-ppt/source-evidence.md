# Plain Business Visual PPT Source Evidence

## Additional Final Deck Evidence

This visual-aware base format inherits the original Plain Business PPT evidence below, then adds lessons from the final Kikai collab seminar deck production.

Key additions from that production pass:

- white base must remain white; richness should come from type scale, information density, image2, SVG, and meaningful card media
- body groups must be centered inside the available body zone, not against the full slide canvas
- title, lead, body, card headings, and card body sizes need explicit projection-oriented roles
- Bordeaux is valuable for emphasis and authority, but should not become a large background surface
- Gold/Orange works best as navigation, rule, number, and side-bar accent
- image2 should be generated per page or per card with explicit aspect ratio; shared generic images create quality drift
- SVG should explain relationships such as flow, gate, loop, branch, risk, comparison, and system layers; generic graph or shield decorations should be avoided
- tables should be made denser with comments, implications, or row-fit changes instead of leaving empty rows
- closing slides should remove scattered decorative SVG and focus on the final message or CTA

The detailed slide-by-slide learning map is in `visual-policy.md`.
