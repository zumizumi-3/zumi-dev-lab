...
- Chatwork由来のタスク候補一覧
- Zoom録画/文字起こし、手動登録タスクの混在表示
- 企業名、発生元、元リンク、登録日、期限表現、解釈後の期限
- 会話コンテキストとAI判定理由
- タスク名、詳細、期限、優先度、反映先の編集
- 採用、保留、却下の状態変更
- 検索、ステータスフィルタ、ソースフィルタ

## 次に本実装へ移す場所

- `candidates`: DBの`task_candidates`に置き換え
- `messages`: Chatwork Webhookで保存した`messages`に置き換え
- `sourceType`: Chatwork、Zoom、手動入力などの発生元分類に置き換え
- `dueText` / `dueInterpreted`: AIの期限解釈結果に置き換え
